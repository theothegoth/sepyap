...
export class IngestService {
  ...
  private async saveProduct(rawItem: any, transactionalEntityManager?: any) {
    ...
    if (marketProduct) {
      ...
      try {
        await this.searchService.indexMarketProduct(marketProduct);
      } catch (e) {
        this.logger.warn(`Failed to index market product ${marketProduct.id}: ${e.message}`);
      }
      ...
      try {
        ...
        this.logger.warn(`Failed to index new market product ${marketProduct.id}: ${e.message}`);
      }
